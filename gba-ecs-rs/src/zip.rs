...
pub struct ZippedQuery2<'a, T1, T2> {
    ...
    len: usize,
    shortest_active_indices: &'a Vec<usize>,
    _phantom: PhantomData<&'a ()>,
}

impl<'a, T1: 'a, T2: 'a> ZippedQuery2<'a, T1, T2> {
    fn new(container1_full: &'a VecComponentContainer<T1>, container2_full: &'a VecComponentContainer<T2>) -> Self {
        ...
        let shortest_active_indices = if container1_full.active_indices.len() <= container2_full.active_indices.len() {
            &container1_full.active_indices
        } else {
            &container2_full.active_indices
        };
    ...
    where
        F: FnMut(usize, &'a mut T1, &'a T2),
    {
        for &index in self.shortest_active_indices {
            unsafe {
...
impl<'a, T1: 'a, T2: 'a, T3: 'a> ZippedQuery3<'a, T1, T2, T3> {
    fn new(
        container1_full: &'a VecComponentContainer<T1>,
        container2_full: &'a VecComponentContainer<T2>,
        container3_full: &'a VecComponentContainer<T3>,
    ) -> Self {
        let container1 = &container1_full.container;
        let container2 = &container2_full.container;
        ...
        let shortest_active_indices = if container1_full.active_indices.len() <= container2_full.active_indices.len()
            ...
            &container3_full.active_indices
        };

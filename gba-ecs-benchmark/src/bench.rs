...
static mut BENCHMARKS: [Option<BenchEntry>; MAX_BENCHMARKS] = [None; MAX_BENCHMARKS];
static mut TIMER_STACK: [Option<TimerFrame>; MAX_STACK_DEPTH] = [None; MAX_STACK_DEPTH];
...
pub fn init(timers: &mut Timers) {
    ...
    // Setup timer2 as the lower 16-bit counter
    timers.timer2.set_enabled(true);
    
    // Setup timer3 as the upper 16-bit counter (cascaded from timer2)
    timers.timer3.set_cascade(true);
    timers.timer3.set_enabled(true);
    
    agb::println!("[BENCH] Timer setup complete - timer2: {}, timer3: {}", 
                 timers.timer2.value(), timers.timer3.value());
}
...
pub fn start(tag: &'static str) {
    unsafe {
        if let Some(timers_ptr) = TIMERS {
            ...
            agb::println!("START! {} {}", start_time.0, start_time.1);
...
pub fn stop(tag: &'static str) {
    unsafe {
        if let Some(timers_ptr) = TIMERS {
            ...
            agb::println!("STOP! {} {}", end_time.0, end_time.1);
...
fn calc_elapsed(start: (u16, u16), end: (u16, u16)) -> u32 {
    ...
    if elapsed > 50_000_000 {
        // More than ~50M cycles is suspicious for short operations (increased from 1M)
        agb::println!("[BENCH DEBUG] Discarded large elapsed: {} (start: {:08x}, end: {:08x})", elapsed, start_val, end_val);
        0 // Return 0 for obviously invalid measurements
    } else {
        agb::println!("[BENCH DEBUG] Elapsed: {} cycles (start: {:08x}, end: {:08x})", elapsed, start_val, end_val);
        elapsed
    }
}
